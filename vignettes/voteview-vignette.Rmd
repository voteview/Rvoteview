...
Downloading `r nrow(res)` rollcalls can be quite slow, for now, so be patient. We are working to improve speed.
...
Now we use the `pscl` package to run an ideal point estimation.

```{r defense-ideal, cache=T, message = F}
library(pscl)
defIdeal <- ideal(rc,
                  normalize = T)
```
