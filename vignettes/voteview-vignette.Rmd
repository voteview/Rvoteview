...
Also, see usage below in the regression analysis example.
...
## Analyzing the impact of reelection on ideal points

The package also provides ideal points estimated by TODO:FINDALGORITHM for each legislator in each congress. We can use this information to track how senators who faced reelection changed relative to senators who did not face reelection. Let's examine the 2008 election. For now we need to get at least one Senate roll call from the 110th and 111th congresses for this analysis because the stored ideal points TODO:NAME FOR SCORES constate at the legislator-session level. In the future it may be possible to search directly for legislators and congressional sessions.
...
## Extract one roll call
sen110id <- senateRes[senateRes$session == 110, "id"][1]
sen111id <- senateRes[senateRes$session == 111, "id"][1]

senate2010dld <- voteview_download(c(sen110id, sen111id))
```

Now let's get the long data frame of legislator-congresses and cast it to be at the legislator level but retaining the ideal points estimated for each congress.

```{r melt-senate}
## This line takes our legis.long.dynamic df and adds to it "session" from the
## vote.data df
senDf <- melt_rollcall(senate2010dld, votecols = "session")
head(senDf)

senate2010dld$legis.long.dynamic$icpsr
## Let's drop the POTUS
senDf <- senDf[senDf$state != 99, ]
nrow(senDf)

## Plot ideal points by congress
ggplot(senDf, aes(x = as.numeric(x), y = as.numeric(y), color = party, label = cqlabel)) +
  facet_grid(. ~ session) + 
  geom_text() +
  scale_color_manual("Party", 
                     labels = c("100" = "Democrat",
                                "200" = "Republican",
                                "328" = "Independent"),
                     values = c("200" = "red",
                                "100" = "blue",
...
## First melt the data to get ideal points x and y in one column
senDfLong <- reshape2::melt(senDf, id = c("icpsr", "name", "session", "state",
                                          "cqlabel", "party", "id"))
## Cast the data so that each row is a legislator
senDfWide <- reshape2::dcast(senDfLong,
                             icpsr + name + state + cqlabel + party ~ variable + session,
                             value.var = "value")
head(senDfWide)
```

Now let's add their 2010 election status from an external source.

TODO: it appears all ideal points are identical across congresses
