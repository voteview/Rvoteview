...
```{r defense-ideal-wnominate, eval = F}
library(wnominate)
...
id <- pscl::ideal(rc,
                  d = 2)
```
```{r ideal-lgbt, echo = F, results = 'hide', warnings = F, cache = T}
id <- pscl::ideal(rc,
                  d = 2,
                  maxiter = 5000,
                  thin = 10,
...
```{r}
data(states)
dadtLong <- merge(dadtLong, states[, c("stateMail", "stateName")],
                  by.x = "state_abbrev", by.y = "stateMail")
